...
public:
	...
	//名前でオブジェクトを検索（対象は全体）(戻り値GameObject*)
	//引数：検索する名前
	//戻値：見つけたオブジェクトのアドレス
	GameObject* FindObject(const std::string& name) { return GetRootJob()->FindChildObject(name); }

	//名前でオブジェクトを検索（対象は全体）(戻り値はテンプレート指定)
	//引数：検索する名前
	//戻値：見つけたオブジェクトのアドレス
	template<typename T>
	T* FindObject(const std::string& name) {
		return dynamic_cast<T*>(GetRootJob()->FindChildObject(name));
	}
...
protected:
	//オブザーバーを追加
	void AttachObserver(GameObjectObserver* observer);
	//オブザーバーを削除
	void DetachObserver(GameObjectObserver* observer);
	//登録されているオブザーバー全てに衝突を通知
	void NotifyCollision(GameObject* pTarget);

private:
	//登録されたオブザーバーのリスト
	std::vector<GameObjectObserver*> observers_;
	///////////////////////
};
